...
@section('page-title', __('messages.new_booking'))
    ...
    <div class="col-md-10">
        <div class="card">
            <div class="card-header">
                <i class="bi bi-plus-circle me-1"></i> {{ __('messages.new_booking') }}
            </div>
            <div class="card-body">
                <form action="{{ route('bookings.store') }}" method="POST">
                    ...
                    <div class="card mb-3" style="background: #fafbfc;">
                        <div class="card-body">
                            ...
                            <div id="tour_manual_section" style="display: none;">
                                ...
                                <input type="text" name="tour_manual" id="tour_manual" class="form-control @error('tour_manual') is-invalid @enderror" value="{{ old('tour_manual') }}" placeholder="{{ __('messages.manual_tour_placeholder') }}">
                                @error('tour_manual')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="currency" id="currency" class="form-select @error('currency') is-invalid @enderror" required>
                                ...
                                <option value="BRL" {{ old('currency') == 'BRL' ? 'selected' : '' }}>{{ __('messages.brl_real') }}</option>
                                <option value="USD" {{ old('currency') == 'USD' ? 'selected' : '' }}>{{ __('messages.usd_dollar') }}</option>
                                <option value="EUR" {{ old('currency') == 'EUR' ? 'selected' : '' }}>{{ __('messages.eur_euro') }}</option>
                                <option value="ZAR" {{ old('currency') == 'ZAR' ? 'selected' : '' }}>{{ __('messages.zar_rand') }}</option>
                            </select>
                            @error('currency')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="discount_notes" id="discount_notes" class="form-control @error('discount_notes') is-invalid @enderror" rows="2" placeholder="{{ __('messages.discount_notes_placeholder') }}">{{ old('discount_notes') }}</textarea>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                                <option value="pendente" {{ old('status', 'pendente') == 'pendente' ? 'selected' : '' }}>{{ __('messages.status_pending') }}</option>
                                <option value="confirmado" {{ old('status') == 'confirmado' ? 'selected' : '' }}>{{ __('messages.status_confirmed') }}</option>
                                <option value="cancelado" {{ old('status') == 'cancelado' ? 'selected' : '' }}>{{ __('messages.status_cancelled') }}</option>
                                <option value="concluido" {{ old('status') == 'concluido' ? 'selected' : '' }}>{{ __('messages.status_completed') }}</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="notes" id="notes" class="form-control @error('notes') is-invalid @enderror" rows="3" placeholder="{{ __('messages.booking_notes_placeholder') }}">{{ old('notes') }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

                    <div class="section-heading">{{ __('messages.auto_installments') }}</div>

                    <div class="card mb-4" style="background: #fafbfc;">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="num_installments" class="form-label">{{ __('messages.num_installments') }}</label>
                                    <input type="number" name="num_installments" id="num_installments" class="form-control" value="{{ old('num_installments', 0) }}" min="0">
                                    <div class="form-text">{{ __('messages.installments_help') }}</div>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="installment_payment_method" class="form-label">{{ __('messages.payment_method') }}</label>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-lg"></i> {{ __('messages.save_booking') }}
                        </button>
                        <a href="{{ route('bookings.index') }}" class="btn btn-outline-secondary">{{ __('messages.cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>
