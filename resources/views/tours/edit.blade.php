...
@section('title', __('messages.edit_tour'))
@section('page-title', __('messages.edit_tour'))
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <i class="bi bi-pencil me-1"></i> {{ __('messages.edit_tour') }}: {{ $tour->name }}
            </div>
            <div class="card-body">
                <form action="{{ route('tours.update', $tour) }}" method="POST">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            ...
                            <select name="type" id="type" class="form-select @error('type') is-invalid @enderror" required>
                                ...
                                <option value="grupo" {{ old('type', $tour->type) == 'grupo' ? 'selected' : '' }}>{{ __('messages.group') }}</option>
                                <option value="privado" {{ old('type', $tour->type) == 'privado' ? 'selected' : '' }}>{{ __('messages.private') }}</option>
                                <option value="agencia" {{ old('type', $tour->type) == 'agencia' ? 'selected' : '' }}>{{ __('messages.agency') }}</option>
                                <option value="influencer" {{ old('type', $tour->type) == 'influencer' ? 'selected' : '' }}>{{ __('messages.influencer') }}</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="default_currency" id="default_currency" class="form-select @error('default_currency') is-invalid @enderror">
                                <option value="">{{ __('messages.select') }}</option>
                                <option value="BRL" {{ old('default_currency', $tour->default_currency) == 'BRL' ? 'selected' : '' }}>{{ __('messages.brl_real') }}</option>
                                <option value="USD" {{ old('default_currency', $tour->default_currency) == 'USD' ? 'selected' : '' }}>{{ __('messages.usd_dollar') }}</option>
                                <option value="EUR" {{ old('default_currency', $tour->default_currency) == 'EUR' ? 'selected' : '' }}>{{ __('messages.eur_euro') }}</option>
                                <option value="ZAR" {{ old('default_currency', $tour->default_currency) == 'ZAR' ? 'selected' : '' }}>{{ __('messages.zar_rand') }}</option>
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                            <option value="ativo" {{ old('status', $tour->status) == 'ativo' ? 'selected' : '' }}>{{ __('messages.active') }}</option>
                            <option value="inativo" {{ old('status', $tour->status) == 'inativo' ? 'selected' : '' }}>{{ __('messages.inactive') }}</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('tours.show', $tour) }}" class="btn btn-outline-secondary">{{ __('messages.cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>
